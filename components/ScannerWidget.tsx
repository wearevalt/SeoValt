      ...
      <div style={containerStyle}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          <input
            ...
            style={{ flex: 1, padding: '14px 18px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)', color: '#fff', fontSize: '15px', outline: 'none' }}
          />
          <button
            onClick={handleScan}
            style={{ padding: '14px 22px', borderRadius: '10px', background: 'linear-gradient(135deg, #00e5a0, #00c07f)', color: '#000', border: 'none', fontWeight: 700, fontSize: '15px', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            Analyser →
          </button>
        </div>
    ...
    return (
      <div style={{ ...containerStyle, textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '8px', height: '4px', marginBottom: '16px', overflow: 'hidden' }}>
          <div style={{ height: '100%', background: 'linear-gradient(90deg, #00e5a0, #00c07f)', borderRadius: '8px', width: '60%' }} />
        </div>
      ...
      <div style={containerStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
          {result.visibleErrors.map((err: any) => (
            <div key={err.id} style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '10px', padding: '12px 16px' }}>
          ...
          {result.lockedErrors.length > 0 && (
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: '10px', padding: '12px 16px', textAlign: 'center' }}>
        ...
        <a href="/#pricing" style={ctaStyle}>
          {result.upgradeCta.sub}
        </a>
        <button
          onClick={() => { setState('idle'); setUrl(''); setResult(null); }}
          style={{ width: '100%', padding: '10px', borderRadius: '10px', background: 'transparent', color: 'rgba(255,255,255,0.25)', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', fontSize: '13px' }}
        >
